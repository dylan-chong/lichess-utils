...
describe('squareToPixels validation', () => {
  it('throws for empty square string', () => {
    expect(() => squareToPixels('', 78, 'white')).toThrow('Invalid square notation')
  })

  it('throws for single character', () => {
    expect(() => squareToPixels('a', 78, 'white')).toThrow('Invalid square notation')
  })

  it('throws for invalid file', () => {
    expect(() => squareToPixels('z5', 78, 'white')).toThrow('Invalid file')
  })

  it('throws for invalid rank (too high)', () => {
    expect(() => squareToPixels('a9', 78, 'white')).toThrow('Invalid rank')
  })

  it('throws for invalid rank (too low)', () => {
    expect(() => squareToPixels('a0', 78, 'white')).toThrow('Invalid rank')
  })
})
